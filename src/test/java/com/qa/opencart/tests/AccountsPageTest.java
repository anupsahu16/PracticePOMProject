...
public class AccountsPageTest extends BaseTest{
	...
	public void accPageTitleTest(){
		...
		Assert.assertEquals(title, Contants.HOME_PAGE_TITLE, Error.ACC_PAGE_MISMATCHED_ERROR);
	}
	...
	@Test(priority = 3)
	public void accPageSectionsCount(){
		Assert.assertEquals(accPage.getAccountPageHeadersCount(),
				Contants.ACC_PAGE_SECTIONS_COUNT , Error.ACC_PAGE_SECTION_ERROR);
	...
	public void accPageSectionsTest(){
		List<String> actualAccSecList = accPage.getAccountHeadersList();
		System.out.println(actualAccSecList);
		
		Assert.assertEquals(actualAccSecList, Contants.expectedAccSecList());
	...
	public void selectProductTest(){
		...
		softAssert.assertEquals(productInfoPage.getProductImagesCount(), Contants.PRODUCT_IMAGES_COUNT_MACBOOK);
